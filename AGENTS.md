...
- `codex/skills/` — group `core`. Local skills that define the harness.
- `codex/aside/` — group `aside`. Skills kept here but unrelated to the core flow.
- `codex/vendor/` — group `vendor`. Upstream snapshots. Do not edit them here;
  change the upstream repository, then refresh the snapshot and the pinned
  commit in `SOURCES.lock.json`.

Adding a skill means three things, all in the same commit: the directory with
`SKILL.md`, an `agents/openai.yaml`, and a `manifest.json` entry.
...
- `agents/openai.yaml` sets `interface.display_name`, `short_description`, and
  `default_prompt`. Skills that must never fire on their own also set
...
```sh
./install.sh --dry-run
CODEX_HOME="$(mktemp -d)" ./install.sh --only <skill>   # then diff -rq against the source
```

Installation uses `rsync --delete`, so it replaces a same-named skill already
present in `~/.codex/skills`.
